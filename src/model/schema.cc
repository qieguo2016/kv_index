...
StatusOr<CompiledRowLayout> CompiledRowLayout::Compile(
  ...
  layout.presence_bitmap_bytes_ =
      base::AlignUp((active_fields.size() + 7U) / 8U, 8U);
  ...
  for (FieldLayout& field : layout.fields_) {
    ...
    fixed_cursor = base::AlignUp(fixed_cursor, FieldTypeAlignment(field.type));
    field.slot_offset = fixed_cursor;
    fixed_cursor += field.value_width;
  }

  layout.ref_area_offset_ = base::AlignUp(fixed_cursor, 8U);
  ...
  layout.row_slot_size_ = base::AlignUp(ref_cursor, 8U);
