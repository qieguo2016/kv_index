...
Status AppendString(std::vector<std::byte>* out, const std::string& value) {
  if (value.size() > UINT32_MAX) {
    return Status::InvalidArgument("artifact string is too large");
  }
...
StatusOr<std::vector<std::byte>> SerializeSchema(
  ...
  if (layout.fields().size() > UINT32_MAX) {
    return Status::InvalidArgument("artifact has too many fields");
  }
...
StatusOr<std::vector<std::byte>> SerializeSourceProgress(
    const std::vector<ArtifactSourceProgressBuildSpec>& progress) {
  std::vector<std::byte> bytes;
  ...
  for (const ArtifactSourceProgressBuildSpec& entry : progress) {
...
StatusOr<std::shared_ptr<const store::OwnedSnapshotBacking>> BuildShardSnapshot(
    const ArtifactBuildSpec& spec, const ArtifactShardBuildSpec& shard) {
  ...
  for (const ArtifactRow& row : shard.rows) {
...
Status WriteArtifact(const std::string& path, const ArtifactBuildSpec& spec) {
  ...
  if (sections.size() != section_count) {
    return Status::Internal("artifact section count mismatch");
  }
  if (const Status status = WriteLittleEndian<std::uint64_t>(
