...
class ByteReader {
 public:
  ...
  StatusOr<T> ReadInteger() {
    ...
    auto value = internal::ReadLittleEndian<T>(bytes_, offset_);
    if (!value.ok()) {
      return value.status();
    }
    offset_ += sizeof(T);
...
StatusOr<T> ReadExactInteger(std::span<const std::byte> bytes) {
  ...
  return internal::ReadLittleEndian<T>(bytes, 0);
}
...
Status WriteArenaStringListField(const FieldLayout& field,
  ...
  for (std::size_t i = 0; i < values.size(); ++i) {
    ...
    const Status ref_status = internal::WriteValueRef16(
        internal::ValueRef16{
            .offset = value_offset,
  ...
  return internal::WriteValueRefField(
      field,
      internal::ValueRef16{
          .offset = refs_offset,
...
Status WriteScalarFieldValue(const FieldLayout& field,
  ...
  switch (field.type) {
    case FieldType::kInt8: {
      ...
      return internal::WriteScalarField(field, *value, encoded);
    }
    ...
    case FieldType::kString: {
      ...
      if (field.encoding == FieldEncoding::kArena) {
        return internal::WriteArenaStringField(field, *value, encoded);
      }
      if (field.encoding == FieldEncoding::kDictionary) {
        ...
        return internal::WriteDictionaryStringField(field, id, encoded);
      }
...
Status WriteArenaScalarList(const FieldLayout& field,
                            ...
                            internal::EncodedRow* encoded) {
  return internal::WriteArenaListField<T>(
...
Status WriteListDictionaryScalarList(const FieldLayout& field,
  ...
  auto payload = internal::EncodeScalarList(values);
  if (!payload.ok()) {
    return payload.status();
  }
  ...
  return internal::WriteListDictionaryField(
  ...
  switch (field.type) {
    ...
    case FieldType::kString: {
      ...
      if (field.encoding == FieldEncoding::kElementDictionary) {
        ...
        return internal::WriteElementDictionaryStringListField(
...
StatusOr<internal::EncodedRow> EncodeForLayout(
  ...
  auto encoded = internal::CreateEncodedRow(*layout);
  ...
  auto validation = internal::MaterializeRow(layout, encoded);
  if (!validation.ok()) {
...
std::size_t ShardFor(std::uint64_t primary_key,
  ...
  const std::uint64_t hash =
      StableHash64(primary_key, route.hash_seed, route.hash_version);
