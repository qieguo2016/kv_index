...
class ShardState {
 public:
  struct Layers {
    ...
    std::optional<store::CompactDeltaSnapshot> compact_delta;
    std::optional<store::FullSnapshotView> full_snapshot;
  };
  ...
  const std::shared_ptr<const store::RealtimeDeltaAtomicTable>& realtime_delta()
      const noexcept {
    return realtime_delta_;
  }
  const std::optional<store::CompactDeltaSnapshot>& compact_delta() const noexcept {
    return compact_delta_;
  }
  const std::optional<store::FullSnapshotView>& full_snapshot() const noexcept {
 ...
 private:
  ...
  std::shared_ptr<const store::RealtimeDeltaAtomicTable> realtime_delta_;
  std::optional<store::CompactDeltaSnapshot> compact_delta_;
