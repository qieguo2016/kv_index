...
using kv_index::core::ArtifactSection;
using kv_index::core::ArtifactSectionType;
using kv_index::core::Fnv1a64;
using kv_index::core::kArtifactFormatVersion;
using kv_index::core::kArtifactGlobalShardId;
using kv_index::core::kArtifactHeaderSize;
using kv_index::core::kArtifactMagic;
using kv_index::core::kArtifactSectionEntrySize;
...
StatusOr<std::vector<std::byte>> SerializeRowPayloads(
    const std::vector<core::OwnedSnapshotRowPayload>& payloads) {
  std::vector<std::byte> bytes;
  ...
  for (const core::OwnedSnapshotRowPayload& payload : payloads) {
...
StatusOr<std::shared_ptr<const core::OwnedSnapshotBacking>> BuildShardSnapshot(
    const TestArtifactSpec& spec, const ArtifactShardSpec& shard) {
  core::SnapshotBuilder builder(
      spec.layout, core::SnapshotBuildOptions{.hash_seed = spec.hash_seed,
...
Status WriteTestArtifact(const std::string& path, const TestArtifactSpec& spec) {
  ...
  if (const Status status = WriteLittleEndian<std::uint32_t>(
          static_cast<std::uint32_t>(core::ArtifactChecksumAlgorithm::kFnv1a64),
