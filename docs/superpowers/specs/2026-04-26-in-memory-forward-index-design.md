...
`ForwardIndexOptions` is the SDK initialization entry point. It centralizes serving mode, shard count, delta compaction / full rebase / rebuild cutover thresholds, and Kafka consumer parameters. After construction, `ForwardIndex` owns these options and uses them to create internal `KafkaUpdateConsumer`s and background update coordinators.

There are three serving modes:

- `kFullSnapshotOnly`: load only the mmap-backed full snapshot. It does not create Kafka catch-up realtime delta and does not enable compact delta / internal full rebase. This fits deployments with no realtime updates or deployments that rely entirely on external full refreshes.
- `kFullSnapshotWithRealtimeDelta`: load the full snapshot and write Kafka catch-up / live apply rows into `RealtimeDeltaAtomicTable`; do not enable `CompactDeltaSnapshot`. This fits low-update workloads with frequent external full updates. The artifact must still carry source progress so the realtime layer can catch up safely after a full cutover.
- `kFullSnapshotWithRealtimeDeltaAndCompaction`: use the complete three-layer serving stack: realtime delta, compact delta, and full snapshot. This fits online workloads where updates keep growing and realtime hash probes / append-only memory must be bounded. The legacy name `kRealtimeDelta` remains an alias, and this is the default mode.
...
All shards use the same published container structure; the mode only decides which optional layers exist. Delta compaction, internal `Full Rebase`, and external `AsyncLoad` also use the same generation switch model:
...
In the full mode, the read path remains:

```text
realtime_delta -> compact_delta -> full_snapshot
```

`kFullSnapshotWithRealtimeDelta` degrades this to `realtime_delta -> full_snapshot`; `kFullSnapshotOnly` degrades it to `full_snapshot`. Only optional layer presence and the immutable inputs scanned by the background builder differ between update types.
...
Delta compaction is enabled only in `kFullSnapshotWithRealtimeDeltaAndCompaction` / `kRealtimeDelta` mode. `kFullSnapshotWithRealtimeDelta` has no compact delta layer; it relies on a small realtime delta and more frequent external full `AsyncLoad` operations to bound delta size.

Realtime delta grows with live updates. In the full mode, each shard should compact independently:
...
The full-mode read path remains fixed:
...
`Full Rebase` does not consume an external full watermark and does not switch schema version. During rebase, new Kafka upserts write into the rebase generation realtime delta, which remains the highest-priority overlay after cutover. After publication, the full-mode read path remains:
