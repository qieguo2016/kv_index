...
2. Ensure every Kafka partition for RebuildGeneration shard_i is below the configured lag threshold and has replayed through the safe source position published by Live Apply for shard_i.
...
3. From after the boundary onward, write Kafka upserts to both ActiveGeneration and CompactGeneration.
...
- External rebuild cutover is allowed only after every relevant Kafka partition lag is below threshold.
