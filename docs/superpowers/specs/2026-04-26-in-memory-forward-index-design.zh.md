...
组件面向高并发、读多写少的在线查询场景优化。Kafka upsert 被目标 shard 的 active generation update stream 本地发布成功后，后续查询可以看到新 row；与发布过程并发的查询允许返回旧 row，若观察到 realtime slot 正在创建也允许直接返回 miss。读路径必须读取一个完整 row version，不能看到部分写入的 value。
...
- 第一版不提供跨所有 shard 的全局多 key 快照隔离。核心语义是单 key 完整 row 读取，不要求 realtime delta 线性一致。
...
索引提供单 key 完整 row 读取语义，但 realtime delta 读路径不追求线性一致：

- 一次 lookup 只观察一个 shard state。
- 在该 shard 内，lookup 看到一个完整 row version。
- lookup 永远不会看到部分编码完成的 row。
- lookup 与 realtime slot 创建并发时，如果观察到 `reserved`，realtime delta 层可以直接返回 miss，不等待、不重试；上层仍可继续查询 compact/full 层。
- lookup 与已有 key 的 realtime row pointer 更新并发时，可以看到旧的完整 row version。
...
4. state 为 reserved 时直接返回 miss。
5. state 为 occupied 时比较 primary_key。
6. key 相等后 acquire-load latest_row 并返回 RowRef。
7. 遇到 empty group 且 probe 终止时返回 miss。
```

读路径不持有 shard 级读锁，也不访问正在初始化的 slot。若观察到 `reserved`，说明 writer 正在创建 slot；`RealtimeAtomicHashMap` lookup 直接返回 miss，不跳过、不等待、不重试，也不读取 key 或 row pointer。上层可以把它当作 realtime delta miss 继续查询 compact/full 层。这样可能让与 slot 创建并发的查询错过稍后完成发布的新 realtime row，但换取最短读路径和稳定尾延迟。
...
6. CAS 成功的 writer 先把 control byte 从 empty 写成当前 key 的 H2，使 probe chain 保持非空；并发 reader 若命中该 reserved slot，会按 miss 返回。
...
`RealtimeAtomicHashMap` 的一致性目标是完整 row 可见，而不是线性一致查询。slot 创建期间的读可以返回 miss；已有 key 更新期间的读可以 acquire-load 到旧 `RowRef*` 并返回旧 row。写路径只需要保证不会发布部分初始化的 slot 或部分编码的 row。
...
第 4 步完成后，后续读请求可以看到新 row。与第 4 步并发的读请求可能看到旧 row；如果 realtime delta lookup 观察到新 slot 仍处于 `reserved`，可以直接返回 realtime miss 并继续查询 compact/full 层。旧 row memory 不在写入路径上立即回收；被替换的 row 会保留在 append-only arena 中，直到 realtime table generation 完成 compaction，并且 reader epoch 证明没有查询还可能引用旧 row 后再释放。
...
本节描述外部 `AsyncLoad` 路径：离线流程产出新的 sharded full artifact 后，SDK 集成方调用 `ForwardIndex::LoadAsync` 触发全量更新。
...
- realtime delta 使用 CAS slot creation 和 atomic row pointer publication 保证只发布完整 row；并发读允许在 slot 创建时返回 miss，或在 row pointer 更新时返回旧 row。
...
- realtime delta 本地发布完成后可见；与发布并发的读允许看到旧 row，slot 创建中允许返回 realtime miss。
